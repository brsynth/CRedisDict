...
class TestCRedisDict(unittest.TestCase):
    ...
    def __init__(self, testname, db_host):
        super(TestCRedisDict, self).__init__(testname)
        self.redis = StrictRedis(host=db_host, port=6379, db=0, decode_responses=True)
        self.redis.flushall()
...
if __name__ == '__main__':
    ...
    test_names = test_loader.getTestCaseNames(TestCRedisDict)

    suite = unittest.TestSuite()
    for test_name in test_names:
        suite.addTest(TestCRedisDict(test_name, 'db'))

    result = unittest.TextTestRunner().run(suite)
    sys_exit(not result.wasSuccessful())
